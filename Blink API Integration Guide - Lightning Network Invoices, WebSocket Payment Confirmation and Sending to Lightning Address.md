...
async function createBTCInvoice(walletId, amountSats, memo) {
  ...
  return {
    paymentRequest:
      data.data.lnInvoiceCreateOnBehalfOfRecipient.invoice.paymentRequest,
    expiryMinutes: 15,
  };
}
```
...
async function createUSDInvoice(walletId, amountCents, memo) {
  ...
  return {
    paymentRequest:
      data.data.lnUsdInvoiceCreateOnBehalfOfRecipient.invoice.paymentRequest,
    expiryMinutes: 5,
  };
}
```
  ...
  wsClient.onmessage = (event) => {
    try {
      ...
      if (data.type === "data" && data.payload && data.payload.data) {
        const paymentStatus =
          data.payload.data.lnInvoicePaymentStatusByPaymentRequest;
...
class BlinkPaymentProcessor {
  ...
  async processPayment(username, amount, currency = "sats", memo = "Payment") {
    try {
      ...
      if (currency !== "sats" && currency !== walletInfo.currency) {
        finalAmount = await this.convertCurrency(
          amount,
          currency,
          walletInfo.currency
        );
      ...
      console.log(
        "Invoice created:",
        invoice.paymentRequest.substring(0, 30) + "..."
      );
  ...
  monitorPayment(paymentRequest, onPaymentConfirmed) {
    ...
    this.websocket = this.subscribeToPaymentStatus(
      paymentRequest,
      onPaymentConfirmed
    );
  }
  ...
  async convertCurrency(amount, fromCurrency, toCurrency) {
    ...
    if (toCurrency === "BTC") {
      // Convert to satoshis
      return Math.round(
        amount * rate.btcSatPrice.base * Math.pow(10, rate.btcSatPrice.offset)
      );
    } else if (toCurrency === "USD") {
      // Convert to cents
      return Math.round(
        amount * rate.usdCentPrice.base * Math.pow(10, rate.usdCentPrice.offset)
      );
    }
...
processor
  .processPayment("satoshi", 1000, "sats", "Donation via website")
  .then((invoice) => {
...
```graphql
subscription LnInvoicePaymentStatusByPaymentRequest(
...
class BlinkService {
  ...
  public async sendBitcoinReward(params: RewardParams): Promise<RewardResult> {
    ...
    return {
      ...
      transactionId: `payment_${Date.now()}_${Math.random()
        .toString(36)
        .substring(2, 10)}`,
    };
  }
...
export const checkLightningAddressExists = async (
  req: Request,
  res: Response
) => {
...
```typescript
export const checkAndMarkPaymentInProgress = async (
  paymentId: string
): Promise<boolean> => {

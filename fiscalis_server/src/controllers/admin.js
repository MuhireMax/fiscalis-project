...
async function listApplications(req, res) {
  ...
  if (search) {
    sql +=
      " AND (a.receipt_number LIKE ? OR c.full_name LIKE ? OR c.national_id LIKE ?)";
    const like = `%${search}%`;
...
async function updateStatus(req, res) {
  ...
  const [[app]] = await pool.query(
    "SELECT id, status FROM applications WHERE uuid = ?",
    [req.params.uuid]
  );
  ...
  if (!allowed.includes(newStatus)) {
    return fail(
      res,
      `Cannot transition from '${app.status}' to '${newStatus}'. Allowed: ${
        allowed.join(", ") || "none"
      }`,
      400
    );
  ...
  await pool.query(`UPDATE applications SET ${setClause} WHERE id = ?`, [
    newStatus,
    officerId,
    notes || null,
    app.id,
...
async function getStats(req, res) {
  ...
  try {
    walletBalance = await blinkService.getWalletBalance();
  } catch {
    /* non-fatal */
  }
